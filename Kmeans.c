...
/* perform k-means algorithm give initial mean estimates */    
{
  ...
  cumsum[0] = histo[0];
  for (int i = 1; i < 256; i++) cumsum[i] = cumsum[i-1] + histo[i];
  for (int i = 0; i < 256; i++) cumsum[i] = (long) ROUND(1000.0*(double)cumsum[i]/(double)cumsum[255]);
  for (int i = 0; i < 256; i++) if ((cumsum[i] <= 10) || (cumsum[i] >= 990)) histo[i] = 0;
...
double Kmeans(double *src, unsigned char *label, unsigned char *mask, int NI, int n_clusters, double *voxelsize, int *dims, int thresh_mask, int thresh_kmeans, int iters_nu, int pve, double bias_fwhm)
{
  ...
  long   n[MAX_NC];
  long   vol;
  int    subsample, masked_smoothing;
  ...
  for (n_classes=2; n_classes <= n_classes_initial; n_classes++) {
    ...
    for (int k = 0; k < n_classes-1; k++) {
      for (long i = n_classes-1; i > k+1; i--) mean[i] = Mu[i-1];
      mean[k+1] = Mu[k] + eps;  mean[k] = Mu[k] - eps;
      for (long i = 1; i < k; i++) mean[i] = Mu[i];
    ...
    for (long i = 0; i < n_classes; i++) Mu[i] = mu[i];     
  }
  ...
  if (iters_nu > 0) {
    ...
    for (int j = 0; j < iters_nu; j++) {  
      for (long i = 0; i < vol; i++) {
        ...
        for(long i=0; i<3; i++) fwhm[i] = bias_fwhm;
      ...
      if (e >= last_err)  {
        ...
        if ((count_err > 1) || (((last_err-e)/e < 0.001) && ((last_err-e)/e > 0))) {
          ...
          for (long i = 0; i < vol; i++) src[i] = src_bak[i];
          break;
      }
