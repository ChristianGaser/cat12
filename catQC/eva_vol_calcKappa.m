function varargout = eva_vol_calc_kappa(varargin)
...
% loop  
  ...
  % data evaluation
    for i=1:n 
      ...
      % realginment
      if any(V(i).dim ~= Vref(Vrefi).dim) || opt.realign>1 %|| any(V(i).mat(:) ~= Vref(Vrefi).mat(:)) 
        [pp,ff,ee] = spm_fileparts(V(i).fname);
        if opt.realign
          Vir = [tempname ee];
          try
            copyfile(V(i).fname,Vir); warning off; 
            if 1
              realigntxt = evalc([ ....
