...
for i in ${@}; do
  ...
  if [ "${count}" -eq "0" ]; then
    ...
    # use cross-sectional pipeline, single files already processed 
    if [ "${count}" -gt "0" ]; then
        exit
      echo  ${cat12_dir}/cat_batch_cat.sh $list -p 4 $fg --matlab $matlab --defaults $default $no_surf $rp --bids_folder $bids_folder_cross --logdir $log_folder
    #  find ${full_path_bids_folder_cross}  -name '*.nii' -type f -exec gzip {} +
    #  find ${full_path_bids_folder_cross}  -name '*.gii' -type f -exec gzip {} +
    fi    
  fi
