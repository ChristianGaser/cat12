...
%  ------------------------------------------------------------------------
  ...
  if 0 % enter value
    ...
    % choose by menu
    nproc           = cfg_menu;
    nproc.labels    = {'no multi-threading',sprintf('quater power (%d processes)',...
      floor(cores/4)),sprintf('half power (%d processes)',floor(cores/2)),...
      sprintf('all-1 (%d processes)',cores-1),sprintf('all (%d processes)',cores)};
