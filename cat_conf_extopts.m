...
reduce_mesh.labels  = { ...
  'No reduction, optimal refinement (0)',...
  'SPM approach (1)',...
  'MATLAB approach (2)',...
  'MATLAB approach external (3)',...
  'No reduction, detailed refinement (4)',...  
};
reduce_mesh.values  = {0 1 2 3 4};
reduce_mesh.val     = {3}; 
reduce_mesh.help    = {
  ['Surface reduction is essential for speed and also an equaly distributed meshes. ' ...
   ...
   'Without reduction, surface refinement is limited and the mesh resolution varies locally more than with more the detailed refinement. ' ...
   'The detailed refinement uses the same values for surface refinement ' ...
   '(as the options with mesh reducution) but without reduction, resulting in much longer processing times. ' ... 
   'However, both SPM and MATLAB function can result in fatal MATLAB crashs that can not be caught. ' ...
   'Therefore, a alternative MATLAB option is available that calls reducepatch in an external MATLAB. ' ...
    ]
   ''
};
