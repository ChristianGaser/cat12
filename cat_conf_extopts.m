...
SRP         = cfg_menu;
SRP.tag     = 'SRP';
SRP.name    = 'Surface reconstruction pipelines';
if ~expert
  SRP.labels  = {...
    'CS1 without SIC',...
    'CS2 with SIC',... 
...
elseif expert > 1
  SRP.labels  = {...
    ...
    'CS1 with SIC with optimizationn (12)',... 
    'CS2 without SIC (20)',...
    'CS2 with SIC without optimization (21)',... 
    'CS2 with SIC with optimizationn (22)',... 
    };
...
SRP.help    = {
  ['CAT uses the projection-based thickness approach (PBT; Dahnke et al., 2012) to reconstruct the central surface.  ' ...
   'In 2020, we extensively revised the surface reconcontruction pipeline (SRP), resulting in a new reconstuction scheme (CS2) that supports better control of the mesh resolution and runtime.  ' ... 
   ...
   'Although these overlaps are usually not a problem in structural analyses, SIs are still inoptimal and may cause issues by mapping 3D information onto the surface.  ' ... 
