...
cat.opts.tol       = 1e-4;                    % SPM preprocesseing accuracy (CAT only!) - 1e-2 very low accuracy (fast); 1e-4 default; 1e-6 very high accuracy (slow)
cat.opts.accstr    = 0.5;                     % SPM preprocesseing accuracy (CAT only!) - 0 very low accuracy (fast) .. 1 very high accuracy (slow); default = 0.5
...
cat.extopts.gcutstr      = 2;    % Strength of skull-stripping:               0 - SPM approach; eps-1 gcut; 2 - new approach; -1 - no skull-stripping (already skull-stripped); default = 2
