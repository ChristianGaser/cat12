%-----------------------------------------------------------------------
% Job for longitudinal batch processing in CAT.
%
...
%   be build on or replace the current LBC function after CAT average 
...
if longTPM
  ...
  matlabbatch{mbi}.spm.tools.cat.tools.createTPMlong.files(1)  = cfg_dep('CAT: Segmentation (current release): rp1 affine Image',...
...
if longmodel && bstr > 0
  ...
  matlabbatch{mbi}.spm.tools.cat.tools.longBiasCorr.segment(1)   = cfg_dep('CAT: Segmentation (current release): Native Label Image',...
...
% #######
  ...
  matlabbatch{mbi}.spm.tools.cat.tools.avg_img.data(1)  = cfg_dep('CAT: Segmentation (current release): Deformation Field',...
...
% #######
  ...
  if lowres
    ...
    for ci = 1:2 % only GM and WM are required for Shooting
      ...
      matlabbatch{mbi}.spm.tools.cat.tools.resize.data(1)     = cfg_dep(sprintf('CAT: Segmentation (current release): p%d Image',ci),...
        substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
        substruct('.','tiss', '()',{ci}, '.','p', '()',{':'}));
    ...
    % Shooting full res
    mbi = mbi + 1; mb_GS = mbi;
    matlabbatch{mbi}.spm.tools.cat.tools.warp.images{1}(1)    = cfg_dep('CAT: Segmentation (current release): p1 Image',...
      substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('.','tiss', '()',{1}, '.','p', '()',{':'}));
    matlabbatch{mbi}.spm.tools.cat.tools.warp.images{2}(1)    = cfg_dep('CAT: Segmentation (current release): p2 Image',...
      substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('.','tiss', '()',{2}, '.','p', '()',{':'}));
    matlabbatch{mbi}.spm.tools.cat.tools.warp.dfile           = {fullfile(fileparts(mfilename('fullpath')),'cat_long_shoot_defaults.m')};
  end
  ...
  for ci = 1:2 + write_CSF
    ...
    matlabbatch{mbi}.spm.tools.cat.tools.defs2.images{1}(1) = cfg_dep(sprintf('CAT: Segmentation (current release): p%d Image',ci),...
      substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('.','tiss', '()',{ci}, '.','p', '()',{':'}));
...
if longmodel
  ...
  for ci = 1:(2 + write_CSF)*(1 + (longmodel==3)) % fill image sets
   ...
   %       substruct('.','vfiles'));
        matlabbatch{mbi}.spm.tools.cat.tools.defs.images(1)  = cfg_dep(sprintf('CAT: Segmentation (current release): p%d Image',ci),...
          substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
          substruct('.','tiss', '()',{ci}, '.','p', '()',{':'}));
      else
        matlabbatch{mbi}.spm.tools.cat.tools.defs.images(1)  = cfg_dep('Apply deformations (many subjects): All Output Files',...
      ...
      if longmodel==3
        matlabbatch{mbi}.spm.tools.cat.tools.defs.images(1)  = cfg_dep(sprintf('CAT: Segmentation (current release): p%d Image',ci - (2 + write_CSF)),...
          substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
    ...
    if ~longmodel || mbfdef(modi,1)>0
      if ( modi == 1 ) ||  ( modi == 2 && (longmodel==2 || longmodel==3) ) % allways print in modi 1 ! ... && (longmodel==1 || longmodel==3) )
        ...
        if longmodel
          ...
          matlabbatch{mbi}.spm.tools.cat.tools.long_report.data_vol(1)    = cfg_dep('CAT: Segmentation (current release): mwp1 Image',...
            substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
            substruct('.','tiss', '()',{1}, '.','mwp', '()',{':'})); 
        end
        if surfaces % && useprior && longmodel % otherwise the indivudal surfaces will not have the same mesh!  
          matlabbatch{mbi}.spm.tools.cat.tools.long_report.data_surf(1)   = cfg_dep('CAT: Segmentation (current release): Left Thickness',...
            substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
        ...
        if cat_get_defaults('extopts.expertgui')>0
          matlabbatch{mbi}.spm.tools.cat.tools.long_report.data_xml(1)      = cfg_dep('CAT: Segmentation (current release): ROI XML File',...
            substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
            substruct('.','catroi')); 
...
if delete_temp
  ...
  if longmodel
    ...
    for ci = 1:2 + write_CSF
      matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep(sprintf('CAT: Segmentation (current release): p%d Image',ci),...
        substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
        substruct('.','tiss', '()',{ci}, '.','p', '()',{':'})); c = c+1;
    end
    % deformation field
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Deformation Field',...
      substruct('.','val', '{}',{mb_cat}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
  ...
  if longmodel || longTPM 
    ...
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation: Native Label Image', ...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), ...
      substruct('()',{1}, '.','label', '()',{':'})); c = c+1;
    % remove average preprocessing data
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): CAT Report PDF',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('.','catreportpdf', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): CAT Report JPG',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('.','catreportjpg', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): CAT Report',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('.','catxml', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): CAT log-file',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
    ...
    if ~dartel
      for ci = 1:2
        matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep(sprintf('CAT: Segmentation (current release): rp%d affine Image',ci),...
          substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
    ...
    if ~write_CSF || ~dartel
      matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): rp3 affine Image',...
        substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
    ...
    if exist('ROImenu','var') && ~isempty(ROImenu)
      matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): ROI XML File',...
        substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
  ...
  if longmodel && surfaces
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Left Central Surface',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','lhcentral', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Left Sphere Surface',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','lhsphere', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Left Spherereg Surface',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','lhspherereg', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Left Thickness',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','lhthickness', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Left Pbt',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','lhpbt', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Right Central Surface',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','rhcentral', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Right Sphere Surface',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','rhsphere', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Right Spherereg Surface',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','rhspherereg', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Right Thickness',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
      substruct('()',{1}, '.','rhthickness', '()',{':'})); c = c+1;
    matlabbatch{mbi}.cfg_basicio.file_dir.file_ops.file_move.files(c) = cfg_dep('CAT: Segmentation (current release): Right Pbt',...
      substruct('.','val', '{}',{mb_catavg}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}),...
