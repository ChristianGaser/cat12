...
% $Id$
  ...
  if job.extopts.expertgui >= 0
    ...
    cat_io_cprintf(col, sprintf('Relative gray matter volume (GMV/TIV):   %6.3f%% (%6.3f / %6.3f ml)\n',...
    ...
    if isfield(qa.subjectmeasures,'dist_thickness')
      cat_io_cprintf(col, sprintf('Gray matter thickness (GMT):           %5.3f %s %4.3f mm\n',...
  ...
  try %#ok<TRYNC>
    ...
    cat_io_xml(cat_io_BIDS(job.BIDS(job.subj),'reportdir','prefix','cat_','ext','.xml'),struct(...
