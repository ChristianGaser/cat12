...
% #####
  ...
  if job.extopts.expertgui >= 0 % RD202508: could maybe be shorter but lets start with this
    ...
    if isfield(qa.qualityratings,'res_ECR')
    ...
    if isfield(qa.qualityratings,'FEC')
  ...
  if isfield(qa.subjectmeasures,'dist_thickness') && ~isempty(qa.subjectmeasures.dist_thickness)
    if job.extopts.expertgui > 1 && isfield(qa.subjectmeasures,'dist_thickness_kmeans')
      ...
      if isfield(qa.subjectmeasures,'dist_thickness_md')
        str{3} = [str{3} struct('name', '\bfThickness (mn/md):','value',...
          sprintf('%4.3f%s%4.3f mm / %4.3f%s%4.3f mm', ...
            qa.subjectmeasures.dist_thickness{1}(1),pm,qa.subjectmeasures.dist_thickness{1}(2), ...
            qa.subjectmeasures.dist_thickness_md{1}(1),pm,qa.subjectmeasures.dist_thickness_md{1}(2)))];
      else
        str{3} = [str{3} struct('name', '\bfThickness:','value',sprintf('%4.3f%s%4.3f mm', ...
          qa.subjectmeasures.dist_thickness{1}(1),pm,qa.subjectmeasures.dist_thickness{1}(2)))];
      end
    end
