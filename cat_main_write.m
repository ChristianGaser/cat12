...
% $Id$
  ...
  mridir = cat_io_BIDS(VT0.fname,job,'mridir'); 
  ...
  cat_io_writenii(VT0,Ym,mridir,'m', ...Dartel
  ...
  cat_io_writenii(VT0,Ym.*(Yp0>0.1),mridir,'m', ... 
  ...
  cat_io_writenii(VT0,Ymi,mridir,'mi', ...
  ...
  cat_io_writenii(VT0,Ymi.*(Yp0>0.1),mridir,'mi', ... 
  ...
  % Yp0
  cat_io_writenii(VT0,Yp0,mridir,'p0',[prefix ' label map'],'uint8',[0,5/255],job.output.label,trans);
  
  % partitioning
  cat_io_writenii(VT0,Yl1,mridir,'cat_','brain atlas map for major structures and sides',...
    'uint8',[0,1],job.output.atlas,trans);

  % class maps 1-3
  fn = {'GM','WM','CSF','head','head','background'};
  for clsi=1:3
    cat_io_writenii(VT0,single(Ycls{clsi})/255,mridir,sprintf('p%d',clsi),...
  ...
  if job.extopts.WMHC>=3 && job.extopts.WMHCstr>0 && ~job.inv_weighting
    cat_io_writenii(VT0,single(Ycls{7})/255,mridir,'p7',[prefix 'WMH tissue map'],'uint8',[0,1/255],...
      min([1 1 0 3],[job.output.WMH.native job.output.WMH.warped ...
      job.output.WMH.mod job.output.WMH.dartel]),trans); % 1 0 0 0
    cat_io_writenii(VT0,single(Ycls{7})/255,mridir,'p7',[prefix 'WMH tissue map'],'single',[0,1],...
  ...
  if job.extopts.SLC>0 && numel(Ycls)>7
    cat_io_writenii(VT0,single(Ycls{8}),mridir,'p8',[prefix 'stroke lesion map'],'uint8',[0,1/255],...
      min([1 1 0 3],[job.output.SL.native job.output.SL.warped ...
      job.output.SL.mod job.output.SL.dartel]),trans); % 1 0 0 0
    cat_io_writenii(VT0,single(Ycls{8}),mridir,'p8',[prefix 'stroke lesion map'],'single',[0,1],...
  ...
  if job.output.intsegments
    if (any(tc(:)) || job.extopts.WMHC==3 && job.extopts.WMHCstr>0 && ~job.inv_weighting) 
      ...
      for clsi=1:3
        cat_io_writenii(VT0,single(Yclsi{clsi})/255,mridir,sprintf('pi%d',clsi),...
          sprintf('%s %s tissue map',prefix, fn{clsi}),'uint8',[0,1/255],...
          min([1 1 0 3],[job.output.(fn{clsi}).native job.output.(fn{clsi}).warped ...
          job.output.(fn{clsi}).mod job.output.(fn{clsi}).dartel]),trans);
        cat_io_writenii(VT0,single(Yclsi{clsi})/255,mridir,sprintf('pi%d',clsi),...
      ...
      % write WMH class maps
      if job.extopts.WMHC>=3 && job.extopts.WMHCstr>0 && ~job.inv_weighting
        cat_io_writenii(VT0,(single(Ywmhp) + single(Ycls{7}))/255,mridir,...
  ...
  if any(cell2mat(struct2cell(job.output.TPMC)'))
    ...
    for ri = 1:numel(reg)
      ...
      for clsi=1:6
        Pbg{clsi}  = fullfile(pp,mridir,sprintf('rp%d%s_%s.nii',clsi,ff,reg{ri}));
  ...
  if job.output.jacobian.warped
    ...
    N      = nifti;
    N.dat  = file_array(fullfile(mridir,['wj_', nam, '.nii']),trans.warped.odim(1:3),...
  ...
  if any(cell2mat(struct2cell(job.output.atlas)'))
    ...
    for ai=1:size(FA,1)
      ...
      if strcmp(job.extopts.species,'human') 
        ...
        try
          copyfile(fullfile(cat_get_defaults('extopts.pth_templates'),[atlas '.csv']),fullfile(mridir,[atlas '_' ff '.csv']));
        catch
          copyfile(fullfile(px,[atlas '.csv']),fullfile(mridir,[atlas '_' ff '.csv']));
        end    
      else
        copyfile(fullfile(px,[atlas '.csv']),fullfile(mridir,[atlas '_' ff '.csv']));
      end
      cat_io_writenii(VT0,Ylai,mridir,[atlas '_'],[prefix ' ' atlas ' original'],...
  ...
  if job.output.warps(1)
      ...
      N         = nifti;
      N.dat     = file_array(fullfile(mridir,['y_', nam, '.nii']),[trans.warped.odim(1:3),1,3],'float32',0,1,0);
      N.mat     = trans.warped.M1;
      N.mat0    = trans.warped.M1;
  ...
  if job.output.warps(2) 
    ...
    % write new output
    Ndef      = nifti;
    Ndef.dat  = file_array(fullfile(mridir,['iy_', nam, '.nii']),[res.image0(1).dim(1:3),1,3],...
                [spm_type('float32') spm_platform('bigend')],0,1,0);
  ...
  if job.output.rmat
    M = trans.affine.A;      save(fullfile(mridir,['t_' , nam, '_affine_reorient.mat']),'M');
    M = inv(trans.affine.A); save(fullfile(mridir,['it_', nam, '_affine_reorient.mat']),'M');
    M = trans.rigid.R;       save(fullfile(mridir,['t_' , nam, '_rigid_reorient.mat' ]),'M');
    M = inv(trans.rigid.R);  save(fullfile(mridir,['it_', nam, '_rigid_reorient.mat' ]),'M');
  end
