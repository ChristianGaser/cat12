          ...
          for i=1:job.nproc
            ...
            if ( ( catSIDlast(i) < catSID(i) )  &&  ( any(cattime>0) || ~isempty(caterr) ) ) || ...
              ...
              if isempty(caterr)
                ...
                % surf vals 
                if job.extopts.expertgui > 0 && ~strcmp(catSRMSE{1},'unknown')
...
function job = update_job(job,verbatlas)
  ...
  if size( job.extopts.atlas , 1 ) > 0
    ...
    if any( [job.extopts.atlas{ contains(spm_file(job.extopts.satlas(:,1),'path',''),'julichbrain3') , 4 }] ) && verbatlas
       cat_io_cprintf('warn',...
         ['-------------------------------------------- \n' ...
