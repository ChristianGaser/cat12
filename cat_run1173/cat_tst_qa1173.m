function varargout = cat_tst_qa1173(action,varargin)
      ...
      for fi=1:numel(Pp0)
        try
          if exist(Po{fi},'file')
            ...
            error('cat_tst_qa1173:noYo','No original image.');
          end
          ...
          if ~isempty(Pm{fi}) && exist(Pm{fi},'file')
            ...
            error('cat_tst_qa1173:noYm','No corrected image.');
          end
  
          res.image = spm_vol(Pp0{fi}); 
          [QASfi,QAMfi,cat_qa_warnings{fi}] = cat_tst_qa1173('cat12',Yp0,Vo,Ym,res,cat_warnings,species,opt);
          ...
          try %#ok<TRYNC>
            ...
            switch e.identifier
              case {'cat_tst_qa1173:noYo','cat_get_defaults1173:noYm','cat_get_defaults1173:badSegmentation'}
                em = e.identifier;
              otherwise
