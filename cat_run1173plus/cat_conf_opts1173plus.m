...
tpm.filter  = 'image';
tpm.ufilter = '.*';
tpm.def     =  @(val)cat_get_defaults1173('opts.tpm', val{:});
tpm.num     = [1 1];
tpm.help    = {
...
ngaus.strtype = 'n';
ngaus.num     = [1 6];
ngaus.def     = @(val)cat_get_defaults1173('opts.ngaus', val{:});
ngaus.help    = {
...
biasstr.def     = @(val)cat_get_defaults1173('opts.biasstr', val{:});
if ~expert
...
biasreg.def    = @(val)cat_get_defaults1173('opts.biasreg', val{:});
if 0
...
biasfwhm.def    = @(val)cat_get_defaults1173('opts.biasfwhm', val{:});
biasfwhm.help   = {
...
if cat_get_defaults1173('opts.biasstr')>0
  bias.val = {biasstr};
else
  bias.val = {biasspm};
end
...
%------------------------------------------------------------------------
warpreg         = cfg_entry;
warpreg.def     = @(val)cat_get_defaults1173('opts.warpreg', val{:});
warpreg.tag     = 'warpreg';
...
affreg.def    = @(val)cat_get_defaults1173('opts.affreg', val{:});
...
samp.strtype = 'r';
samp.num     = [1 1];
samp.def    = @(val)cat_get_defaults1173('opts.samp', val{:});
samp.help   = {
