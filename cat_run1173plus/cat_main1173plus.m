function Ycls = cat_main1173plus(res,tpm,job)
...
if ~isfield(res,'spmpp')
  ...
  % use gcut2
    ...
    %%
    Yb = cat_main_gcut1173(Ym,Yp0>0.1,Ycls,Yl1,false(size(Ym)),vx_vol,...
...
%    Ysrc = Ym * Tth.T3th(5); Tth.T3th = Tth.T3thx * Tth.T3th(5);
  ...
  [Yl1,Ycls,YBG,YMF] = cat_vol_partvol1173(Ymi,Ycls,Yb,Yy,vx_vol,job.extopts,tpm.V,noise);
  ...
  if job.extopts.gcutstr>0 & job.extopts.gcutstr<=1
    ...
    try 
      ...
      [Yb,Yl1] = cat_main_gcut1173(Ymo,Yb,Ycls,Yl1,YMF,vx_vol,job.extopts);
      fprintf('%4.0fs\n',etime(clock,stime));
      if 0
        %% just for manual debuging / development of gcut and gcutstr > remove this in 201709?
        job.extopts.gcutstr=0.5; [Yb05,Yl105] = cat_main_gcut1173(Ym,Yb,Ycls,Yl1,YMF,vx_vol,job.extopts); 
        job.extopts.gcutstr=0.1; [Yb01,Yl101] = cat_main_gcut1173(Ym,Yb,Ycls,Yl1,YMF,vx_vol,job.extopts); 
        job.extopts.gcutstr=0.9; [Yb09,Yl109] = cat_main_gcut1173(Ym,Yb,Ycls,Yl1,YMF,vx_vol,job.extopts);
        ds('d2','',vx_vol,(Yb01 + Yb05+Yb09)/3,Ymi.*(0.2+0.8*Yb01),Ymi.*(0.2+0.8*Yb05),Ymi.*(0.2+0.8*Yb09),50)
      end
    catch %#ok<CTCH>
      fprintf('\n'); cat_warnings = cat_io_addwarning(cat_warnings,'CAT:cat_main_gcut1173:err99','Unknown error in cat_main_gcut1173. Use old brainmask.'); fprintf('\n');
...
%  ---------------------------------------------------------------------
if do_dartel 
  [trans,res.ppe.reg] = cat_main_registration1173(job,res,Ycls,Yy,tpm.M);; 
end
  ...
  if 1
    ...
    if 1 % new
      [Yth1,S,Psurf,EC,defect_size] = cat_surf_createCS(VT,VT0,Ymix,Yl1,Yp0/3,YMF,...
        struct('pbtmethod','pbt2x','interpV',job.extopts.pbtres,'Affine',res.Affine,'surf',{surf},'inv_weighting',job.inv_weighting,...
        'verb',job.extopts.verb,'WMT',0));  
    else
      [Yth1,S,Psurf] = cat_surf_createCS1173(VT,Ymix,Yl1,YMF,...
...
if printCATreport
  ...
  Yp0   = zeros(size(Ym),'single'); Yp0(indx,indy,indz) = single(Yp0b)/255*3; %qa2=qa;
  qa    = cat_tst_qa1173('cat12',Yp0,fname0,Ym,res,cat_warnings,job.extopts.species, ...
...
%  ---------------------------------------------------------------------
  ...
  if ~job.extopts.expertgui
    str(end).value = sprintf('%s / %s / %s',defstrm(job.extopts.LASstr),gcutstr{round(job.extopts.gcutstr+2)},defstrm(job.extopts.cleanupstr)); 
  else
    str(end).value = sprintf('%s(%0.2f) / %s(%0.2f) / %s(%0.2f)',defstrm(job.extopts.LASstr),job.extopts.LASstr,...
      gcutstr{round(job.extopts.gcutstr+2)},job.extopts.gcutstr,defstrm(job.extopts.cleanupstr),job.extopts.cleanupstr); 
  end
  if job.extopts.expertgui 
  ...
  cat_main_reportfig(Ym,Yp0,Psurf,job,res,strs);
end
