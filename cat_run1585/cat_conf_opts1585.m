function opts = cat_conf_opts1585(expert)
...
tpm.filter  = 'image';
tpm.ufilter = '.*';
tpm.def     =  @(val)cat_get_defaults1585('opts.tpm', val{1});
tpm.help    = {
...
tpm.num     = [1 1];
tpm.def     =  @(val)cat_get_defaults1585('opts.tpm', val{:});
if expert>1
  tpm.help  = [tpm.help 
   {
...
ngaus.strtype = 'n';
ngaus.num     = [1 6];
ngaus.def     = @(val)cat_get_defaults1585('opts.ngaus', val{:});
ngaus.help    = {
...
biasstr.def     = @(val)cat_get_defaults1585('opts.biasstr', val{:});
if ~expert
...
biasreg.def    = @(val)cat_get_defaults1585('opts.biasreg', val{:});
if 0
...
biasfwhm.def    = @(val)cat_get_defaults1585('opts.biasfwhm', val{:});
biasfwhm.help   = {
...
if cat_get_defaults1585('opts.biasstr')>0
  bias.val = {biasstr};
else
  bias.val = {biasspm};
end
...
%------------------------------------------------------------------------
warpreg         = cfg_entry;
warpreg.def     = @(val)cat_get_defaults1585('opts.warpreg', val{:});
warpreg.tag     = 'warpreg';
...
affreg.def    = @(val)cat_get_defaults1585('opts.affreg', val{:});
...
samp.strtype = 'r';
samp.num     = [1 1];
samp.def    = @(val)cat_get_defaults1585('opts.samp', val{:});
samp.help   = {
...
tol.def    = @(val)cat_get_defaults1585('opts.tol', val{:}); 
...
accstr.def    = @(val)cat_get_defaults1585('opts.accstr', val{:}); % no cat_defaults entry
if expert
...
if cat_get_defaults1585('opts.accstr')>0
  acc.val  = {accstr};
else
  acc.val  = {accspm};
end
...
redspmres.strtype = 'r';
redspmres.num     = [1 1];
redspmres.def     = @(val)cat_get_defaults1585('opts.redspmres', val{:});
