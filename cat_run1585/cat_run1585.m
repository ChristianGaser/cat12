function varargout = cat_run1585(job)
% Segment a bunch of images
% ______________________________________________________________________
%
%   FORMAT cat_run1585(job)
%
  ...
  for i=1:job.nproc
    ...
    matlab_cmd = sprintf(...
        ['"global cprintferror; cprintferror=1; addpath %s %s %s %s;load %s; ' ...
         ...
         'global cat; cat=cat12def; clear cat; cat_run1585(job); "'],...
...
function job = update_job(job)
  ...
  for ai = 1:size(job.extopts.atlas,1)
    ...
    job.extopts.atlas{ai,4} = job.extopts.atlas{ai,2}<=cat_get_defaults1585('extopts.expertgui') && ...
  ...
  if ~isfield(job.opts,'tol')
    job.opts.tol = cat_get_defaults1585('opts.tol');
  end
  ...
  if isempty(strfind(nam,'MNI152')) && strcmp(job.extopts.species,'human') && cat_get_defaults1585('output.ROI')  %~strcmp(nam,'Template_1_IXI555_MNI152')
  ...
  if isempty( job.extopts.vox ),  job.extopts.vox = cat_get_defaults1585('extopts.vox'); end 
...
function vout = run_job(job)
  ...
  for subj=1:numel(job.channel(1).vols)
    ...
    if cat_io_matlabversion>20072 
      cat_run_newcatch1585(job,tpm,subj); 
    else
      if job.extopts.APP == 1070
        cat_run_job10701585(job,tpm,subj); 
      else
