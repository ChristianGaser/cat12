function  [Ymc,Ymi] = cat_run_job_APP_SPMfinal1585(Po,vout,vx_vol,verb,fstr)
...
%  $Id$
  ...
  if mean(Yo(Ycls{end}(:)>192)) > T3th(2)
    ...
    Yhbgi = cat_vol_approx1585(Yhbg .* Yo,'nn',vx_vol,4,struct('lfO',fwhm));
  ...
  if exist('Ycm','var')
    ...
    Ycmi = cat_vol_approx1585(Ycm.* Yo,'nn',vx_vol,4,struct('lfO',fwhm));
  end
  ...
  Ywi = cat_vol_approx1585(Ytmi,'nn',vx_vol,3,struct('lfO',fwhm));
  if 0
    %%
