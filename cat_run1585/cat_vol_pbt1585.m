function [Ygmt,Ypp,Ymf,Ywmd,Ycsfdc] = cat_vol_pbt1585(Ymf,opt)
% ______________________________________________________________________
%
% Cortical thickness and surface position estimation. 
% 
%   [Ygmt,Ypp,Ywmd,Ycsfd] = cat_vol_pbt1585(Ymf,opt)
%  
...
% default variables and check/set function  
  ...
  def.debug     = cat_get_defaults1585('extopts.verb')>2;
  def.verb      = cat_get_defaults1585('extopts.verb')-1;
  def.pbtlas    = 1; % no/yes
  ...
  switch opt.dmethod
    case 'eidist' 
      ...
      % RD 201908
        ...
        YM2max  = cat_vol_approx1585(YM2max,'nh',resT2.vx_volr,2); % use (slow) high resolutions 
 ...
 %  stime = cat_io_cmd(sprintf('    WM distance (GM - %0.2f): ',mean(vx_vol_org)),'g5','',opt.verb,stime);      
        ...
        YM2min  = cat_vol_approx1585(YM2min,'nh',resT2.vx_volr,2); % use (slow) high resolutions
