...
% if isfield(job,'useprior') && ( isempty(job.useprior) && ~strcmp(job.opts.affreg,'prior') )
 %           stime = cat_io_cmd('Affine registration (longitudinal-development model)','','',1,stime); 
        ...
        if isfield(job,'useprior') && ~isempty(job.useprior) 
          ...
          catxml = spm_file( priorname , 'path', job.BIDS(1).reportdir ,'prefix','cat_','ext','.xml'); 
          catmat = spm_file( priorname , 'path', job.BIDS(1).reportdir ,'prefix','cat_','ext','.mat'); 
      ...
      if job.extopts.APP > 0
          ...
          isSPMtpm = strcmp(job.extopts.species,'human') && ...
            ( strcmp(job.opts.tpm , fullfile(spm('dir'),'tpm','TPM.nii') ) || ...
              ...
              strcmp(job.opts.tpm , fullfile(spm('dir'),'tpm','BlaiottaTPM.nii'))   || ...
              strcmp(job.opts.tpm , fullfile(spm('dir'),'tpm','BlaiottaTPM.nii,1')) || ...
              cat_io_contains( spm_file(job.opts.tpm,'basename') , 'BlaiottaTPM' ) );
          [~,fft] = spm_fileparts(job.opts.tpm{1});
      ...
      if 0 % any(diff(Tth(1:3))<=0)
        ...
        ['CAT is designed to work only on highres T1 images. \n' ...
      ...
      if any( Tth(2:3)<0 ) || job.test_warnings
        cat_io_addwarning([mfilename ':negVal'],sprintf( ...
         ['CAT was developed for images with positive values and \\\\n', ...
