function [Ym,Ybg,WMth,bias] = cat_run_job_APP_SPMinit(job,tpm,ppe,n,ofname,nfname,skullstripped)
...
% $Id$
  ...
  for ix=1:numel(sampx)
    ...
    %try
      ...
      warning on;  
%%
      Pmn    = fullfile(mrifolder,['mn' ff '.nii']); 
      Pmn_ri = fullfile(mrifolder,['mn' ff '_r' num2str(ix) '.nii']);
      Pmn_r0 = fullfile(mrifolder,['mn' ff '_r0.nii']);
      ...
      if (ix==2 && numel(sampx)>2) || (ix==1 && numel(sampx)<=2) || bias(1) > preproc.channel.biasfwhm 
        copyfile(fullfile(mrifolder,['mn' ff '.nii']),Pmn_r0); 
      end
      copyfile(fullfile(mrifolder,['mn' ff '.nii']),nfname);
