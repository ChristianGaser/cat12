function varargout = cat_tst_qa20160204(action,varargin)
      ...
      for fi=1:numel(Pp0)
        try
          if exist(Po{fi},'file')
            ...
            error('cat_tst_qa20160204:noYo','No original image.');
          end
          ...
          if ~isempty(Pm{fi}) && exist(Pm{fi},'file')
            ...
            error('cat_tst_qa20160204:noYm','No corrected image.');
          end
  
          [QASfi,QAMfi,cat_qa_warnings{fi}] = cat_tst_qa20160204('cat12',Yp0,Vo,Ym,'',cat_warnings,species,opt);
          ...
          try %#ok<TRYNC>
            ...
            switch e.identifier
              case {'cat_tst_qa20160204:noYo','cat_tst_qa20160204:noYm','cat_tst_qa20160204:badSegmentation'}
                em = e.identifier;
              otherwise
...
function def=defaults
  ...
  def.output.te  = struct('native',cat_get_defaults('output.te.native'), ...
                          'warped',cat_get_defaults('output.te.warped'), ...
                          'dartel',cat_get_defaults('output.te.dartel'));
  def.output.pc  = struct('native',cat_get_defaults('output.pc.native'), ...
