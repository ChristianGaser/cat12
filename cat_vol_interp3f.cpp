...
inline static int access(int M, int N, int O, int x, int y, int z)
{
  if (x < 0)
    x = 0;
  else if (x >= N)
...
inline static int access_unchecked(int M, int N, int O, int x, int y, int z)
{
  return y + M * (x + N * z);
}

inline static void indices_linear(
    int &f000_i,
    int &f100_i,
    int &f010_i,
    int &f110_i,
...
inline static void indices_cubic(
    int f_i[64],
    const int x, const int y, const int z,
    const mwSize &M, const mwSize &N, const mwSize &O)
{
...
static void interpolate_nearest(float *pO, const float *pF,
...
template <mwSize P>
static void interpolate_nearest_unrolled(float *pO, const float *pF,
                                         const float *pX, const float *pY, const float *pZ,
...
static void interpolate_linear(float *pO, const float *pF,
                               const float *pX, const float *pY, const float *pZ,
                               const mwSize ND, const mwSize M, const mwSize N, const mwSize O, const mwSize P,
                               const float s_x, const float o_x,
                               const float s_y, const float o_y,
                               const float s_z, const float o_z)
{
  ...
  for (mwSize i = 0; i < ND; ++i)
  {
    ...
    for (mwSize j = 0; j < P; ++j)
    {
      ...
      pO[i + j * ND] =
          wz0 * (wy0 * (wx0 * pF[f000_i + j * LO] + wx1 * pF[f100_i + j * LO]) +
                 wy1 * (wx0 * pF[f010_i + j * LO] + wx1 * pF[f110_i + j * LO])) +
          wz1 * (wy0 * (wx0 * pF[f001_i + j * LO] + wx1 * pF[f101_i + j * LO]) +
...
template <mwSize P>
static void interpolate_linear_unrolled(float *pO, const float *pF,
                                        const float *pX, const float *pY, const float *pZ,
                                        const mwSize ND, const mwSize M, const mwSize N, const mwSize O,
                                        const float s_x, const float o_x,
                                        const float s_y, const float o_y,
...
static void interpolate_bicubic(float *pO, const float *pF,
                                const float *pX, const float *pY, const float *pZ,
                                const mwSize ND, const mwSize M, const mwSize N, const mwSize O, const mwSize P,
                                const float s_x, const float o_x,
                                const float s_y, const float o_y,
                                const float s_z, const float o_z)
{
  ...
  for (mwSize i = 0; i < ND; ++i)
  {
    ...
    for (mwSize j = 0; j < P; ++j)
    {
      ...
      pO[i + j * ND] =
          wz0 * (wy0 * (wx0 * pF[f_i[0 + 4 * (0 + 4 * 0)] + j * LO] + wx1 * pF[f_i[1 + 4 * (0 + 4 * 0)] + j * LO] + wx2 * pF[f_i[2 + 4 * (0 + 4 * 0)] + j * LO] + wx3 * pF[f_i[3 + 4 * (0 + 4 * 0)] + j * LO]) +
          ...
          wz1 * (wy0 * (wx0 * pF[f_i[0 + 4 * (0 + 4 * 1)] + j * LO] + wx1 * pF[f_i[1 + 4 * (0 + 4 * 1)] + j * LO] + wx2 * pF[f_i[2 + 4 * (0 + 4 * 1)] + j * LO] + wx3 * pF[f_i[3 + 4 * (0 + 4 * 1)] + j * LO]) +
          ...
          wz2 * (wy0 * (wx0 * pF[f_i[0 + 4 * (0 + 4 * 2)] + j * LO] + wx1 * pF[f_i[1 + 4 * (0 + 4 * 2)] + j * LO] + wx2 * pF[f_i[2 + 4 * (0 + 4 * 2)] + j * LO] + wx3 * pF[f_i[3 + 4 * (0 + 4 * 2)] + j * LO]) +
          ...
          wz3 * (wy0 * (wx0 * pF[f_i[0 + 4 * (0 + 4 * 3)] + j * LO] + wx1 * pF[f_i[1 + 4 * (0 + 4 * 3)] + j * LO] + wx2 * pF[f_i[2 + 4 * (0 + 4 * 3)] + j * LO] + wx3 * pF[f_i[3 + 4 * (0 + 4 * 3)] + j * LO]) +
...
template <size_t P>
static void interpolate_bicubic_unrolled(float *pO, const float *pF,
                                         const float *pX, const float *pY, const float *pZ,
                                         const mwSize ND, const mwSize M, const mwSize N, const mwSize O,
                                         const float s_x, const float o_x,
                                         const float s_y, const float o_y,
...
static InterpolationMethod parseInterpolationMethod(const mxArray *method_string)
{
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  if (Fx && mxGetNumberOfElements(Fx) < 2)
    mexErrMsgTxt("Fx needs at least two elements.");
  if (Fy && mxGetNumberOfElements(Fy) < 2)
    mexErrMsgTxt("Fy needs at least two elements.");
  if (Fz && mxGetNumberOfElements(Fz) < 2)
    mexErrMsgTxt("Fz needs at least two elements.");
  ...
  for (mwSize i = 0; i < mxGetNumberOfDimensions(X); ++i)
    outDims[i] = X_dims[i];
  ...
  if (Fx)
  {
    ...
    case Nearest:
      ...
      {
      case 1:
        interpolate_nearest_unrolled<1>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 2:
        interpolate_nearest_unrolled<2>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 3:
        interpolate_nearest_unrolled<3>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 4:
        interpolate_nearest_unrolled<4>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 5:
        interpolate_nearest_unrolled<5>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 6:
        interpolate_nearest_unrolled<6>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 7:
        interpolate_nearest_unrolled<7>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 8:
        interpolate_nearest_unrolled<8>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 9:
        interpolate_nearest_unrolled<9>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      default:
    ...
    case Linear:
      ...
      {
      case 1:
        interpolate_linear_unrolled<1>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 2:
        interpolate_linear_unrolled<2>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 3:
        interpolate_linear_unrolled<3>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 4:
        interpolate_linear_unrolled<4>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 5:
        interpolate_linear_unrolled<5>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 6:
        interpolate_linear_unrolled<6>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 7:
        interpolate_linear_unrolled<7>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 8:
        interpolate_linear_unrolled<8>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 9:
        interpolate_linear_unrolled<9>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      default:
    ...
    case Cubic:
      ...
      {
      case 1:
        interpolate_bicubic_unrolled<1>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 2:
        interpolate_bicubic_unrolled<2>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 3:
        interpolate_bicubic_unrolled<3>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 4:
        interpolate_bicubic_unrolled<4>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 5:
        interpolate_bicubic_unrolled<5>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 6:
        interpolate_bicubic_unrolled<6>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 7:
        interpolate_bicubic_unrolled<7>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 8:
        interpolate_bicubic_unrolled<8>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      case 9:
        interpolate_bicubic_unrolled<9>(pO, pF, pX, pY, pZ, ND, M, N, O, s_x, o_x, s_y, o_y, s_z, o_z);
        break;
      default:
  ...
  else
  {
    ...
    case Nearest:
      ...
      {
      case 1:
        interpolate_nearest_unrolled<1>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 2:
        interpolate_nearest_unrolled<2>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 3:
        interpolate_nearest_unrolled<3>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 4:
        interpolate_nearest_unrolled<4>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 5:
        interpolate_nearest_unrolled<5>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 6:
        interpolate_nearest_unrolled<6>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 7:
        interpolate_nearest_unrolled<7>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 8:
        interpolate_nearest_unrolled<8>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 9:
        interpolate_nearest_unrolled<9>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      default:
    ...
    case Linear:
      ...
      {
      case 1:
        interpolate_linear_unrolled<1>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 2:
        interpolate_linear_unrolled<2>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 3:
        interpolate_linear_unrolled<3>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 4:
        interpolate_linear_unrolled<4>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 5:
        interpolate_linear_unrolled<5>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 6:
        interpolate_linear_unrolled<6>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 7:
        interpolate_linear_unrolled<7>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 8:
        interpolate_linear_unrolled<8>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 9:
        interpolate_linear_unrolled<9>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      default:
    ...
    case Cubic:
      ...
      {
      case 1:
        interpolate_bicubic_unrolled<1>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 2:
        interpolate_bicubic_unrolled<2>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 3:
        interpolate_bicubic_unrolled<3>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 4:
        interpolate_bicubic_unrolled<4>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 5:
        interpolate_bicubic_unrolled<5>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 6:
        interpolate_bicubic_unrolled<6>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 7:
        interpolate_bicubic_unrolled<7>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 8:
        interpolate_bicubic_unrolled<8>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      case 9:
        interpolate_bicubic_unrolled<9>(pO, pF, pX, pY, pZ, ND, M, N, O, float(1), float(0), float(1), float(0), float(1), float(0));
        break;
      default:
