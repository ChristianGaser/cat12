...
% default variables and check/set function  
  ...
  YM = max(0,min(1,(Ymf-2))); YM(Ymf<=0)=nan; Ywmd   = cat_vol_eidist(YM,max(0,min(1,Ymf/2)),[1 1 1],1,1,0,opt.debug); 
  YM = max(0,min(1,(Ymf-1))); YM(Ymf<=0)=nan; Ycsfdc = cat_vol_eidist(YM,max(0,min(1,Ymf/2)),[1 1 1],1,1,0,opt.debug);   
  ...
  if strcmp(opt.method,'pbt2x')  
    ...
    Ygmt1 = cat_vol_pbtp(Ymf,Ywmd,Ycsfd);   Ygmt1 = cat_vol_median3(Ygmt1,Ygmt1>0,Ygmt1>0,opt.resV/4);
    Ygmt2 = cat_vol_pbtp(4-Ymf,Ycsfd,Ywmd); Ygmt2 = cat_vol_median3(Ygmt2,Ygmt2>0,Ygmt2>0,opt.resV/4);
