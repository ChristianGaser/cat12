function varargout = cat_vol_qa201901(action,varargin)
...
%
% [QAS,QAM] = cat_vol_qa201901(action,varargin)
% 
%
% 1) Use GUI interface to choose segmentation and automatic setting of 
%    original and modified image (if available)
%     [QAS,QAM] = cat_vol_qa201901()                = cat_vol_qa201901('p0')
%
%     [QAS,QAM] = cat_vol_qa201901('p0'[,opt])      - p0 class image
%     [QAS,QAM] = cat_vol_qa201901('p#'[,opt])      - p1,p2,p3 class images
%     [QAS,QAM] = cat_vol_qa201901('c#'[,opt])      - c1,c2,c3 class images
%     [QAS,QAM] = cat_vol_qa201901('*#'[,opt])      - csf,gm,wm class images
%     [QAS,QAM] = cat_vol_qa201901('p0',Pp0[,opt])           - no GUI call
%     [QAS,QAM] = cat_vol_qa201901('p#',Pp1,Pp2,Pp3,[,opt])  - no GUI call
%     [QAS,QAM] = cat_vol_qa201901('c#',Pc1,Pc2,Pc3,[,opt])  - no GUI call
%     [QAS,QAM] = cat_vol_qa201901('c#',Pcsf,Pgm,Pwm,[,opt]) - no GUI call
%
%
...
%     [QAS,QAM] = cat_vol_qa201901('c#+'[,opt])     - c1,c2,c3 class images 
%     [QAS,QAM] = cat_vol_qa201901('*#+'[,opt])     - csf,gm,wm class images
%     [QAS,QAM] = cat_vol_qa201901('p0+',Pp0,Po[,Pm,opt])         - no GUI call
%     [QAS,QAM] = cat_vol_qa201901('p#+',Pp1,Pp2,Pp3,Po[,Pm,opt]) - no GUI call
%     [QAS,QAM] = cat_vol_qa201901('c#+',Pc1,Pc2,Pc3,Po[,Pm,opt]) - no GUI call
%
% 
...
%    [QAS,QAM] = cat_vol_qa201901('cat12',Yp0,Po,Ym,res[,opt])
%
%
  ...
  switch action
    ...
    % segment image cases
      ...
      elseif ( ~exist('Pp0','var') || isempty(Pp0) )
        error('MATLAB:cat_vol_qa201901:inputerror',...
...
%        if strcmpi(spm_check_version,'octave'), warning on; end
  ...
  switch action
      ...
      for fi=1:numel(Pp0)
        try
          ...
          elseif exist(fullfile(pp,[ff ee '.gz']),'file')
            ...
            error('cat_vol_qa201901:noYo','No original image.');
          end
          ...
          %if ~exist(Ym,'var') || round( cat_stat_nanmean(Ym(round(Yp0)==3)) * 100) ~= 100 
            ...
            error('cat_vol_qa201901:noYm','No corrected image.');
          end
          ...
          [QASfi,QAMfi] = cat_vol_qa201901('cat12',Yp0,Vo,Ym,res,species,opt,Pp0{fi});
    ...
    % estimation of the measures for the single case    
      ...
      QAS.software.revision_cat = rev_cat;
      QAS.software.function     = which('cat_vol_qa201901');
