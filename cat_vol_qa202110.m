function varargout = cat_vol_qa202110(action,varargin)
...
%
% [QAS,QAM] = cat_vol_qa202110(action,varargin)
% 
%
% 1) Use GUI interface to choose segmentation and automatic setting of 
%    original and modified image (if available)
%     [QAS,QAM] = cat_vol_qa202110()                = cat_vol_qa202110('p0')
%
%     [QAS,QAM] = cat_vol_qa202110('p0'[,opt])      - p0 class image
%     [QAS,QAM] = cat_vol_qa202110('p#'[,opt])      - p1,p2,p3 class images
%     [QAS,QAM] = cat_vol_qa202110('c#'[,opt])      - c1,c2,c3 class images
%     [QAS,QAM] = cat_vol_qa202110('*#'[,opt])      - csf,gm,wm class images
%     [QAS,QAM] = cat_vol_qa202110('p0',Pp0[,opt])           - no GUI call
%     [QAS,QAM] = cat_vol_qa202110('p#',Pp1,Pp2,Pp3,[,opt])  - no GUI call
%     [QAS,QAM] = cat_vol_qa202110('c#',Pc1,Pc2,Pc3,[,opt])  - no GUI call
%     [QAS,QAM] = cat_vol_qa202110('c#',Pcsf,Pgm,Pwm,[,opt]) - no GUI call
%
%
...
%     [QAS,QAM] = cat_vol_qa202110('c#+'[,opt])     - c1,c2,c3 class images 
%     [QAS,QAM] = cat_vol_qa202110('*#+'[,opt])     - csf,gm,wm class images
%     [QAS,QAM] = cat_vol_qa202110('p0+',Pp0,Po[,Pm,opt])         - no GUI call
%     [QAS,QAM] = cat_vol_qa202110('p#+',Pp1,Pp2,Pp3,Po[,Pm,opt]) - no GUI call
%     [QAS,QAM] = cat_vol_qa202110('c#+',Pc1,Pc2,Pc3,Po[,Pm,opt]) - no GUI call
%
% 
...
%    [QAS,QAM] = cat_vol_qa202110('cat12',Yp0,Po,Ym,res[,opt])
%
%
...
%#ok<*ASGLU>
  ...
  switch action
    ...
    % segment image cases
      ...
      elseif nargin<=5
        ...
        error('MATLAB:cat_vol_qa202110:inputerror',...
  ...
  switch action
      ...
      for fi=1:numel(Pp0)
        try
          ...
          if exist(Po{fi},'file')
            ...
            error('cat_vol_qa202110:noYo','No original image.');
          end
          ...
          elseif 1
            ...
            error('cat_vol_qa202110:noYm','No corrected image.');
          end
          ...
          switch e.identifier
            case {'cat_vol_qa202110:noYo','cat_vol_qa202110:noYm','cat_vol_qa202110:badSegmentation'}
              em = e.identifier;
            otherwise
    ...
    % estimation of the measures for the single case    
      ...
      QAS.software.revision_cat = rev_cat;
      QAS.software.function     = which('cat_vol_qa202110');
...
% ##############
      ...
      Yc  = cat_vol_resize(Yo .* Ycm,'reduceV',vx_volx,res,32,'meanm'); % CSF thr. (minimum to avoid PVE)
      ...
      Ywb = cat_vol_resize( (Yo + min(Yo(:))) .* Ywe,'reduceV',vx_volx,res,32,'meanm') - min(Yo(:));   % for WM inhomogeneity estimation (avoid PVE)
