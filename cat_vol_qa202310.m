...
%        if strcmpi(spm_check_version,'octave'), warning on; end
  ...
  [Yos2,YM2] = cat_vol_resize({Ywn,Ywn>0},'reduceV',vx_vol,4,16,'meanm');
  ...
  if T0th(1)<T0th(2) && NCwc>wcth
    [Yos2,YM2] = cat_vol_resize({Ycn,Ycn>0},'reduceV',vx_vol,4,16,'meanm');
...
%  Yb      = cat_vol_morph(cat_vol_morph(Yp0>2,'l',[10 0.1]),'d',2);
  ...
  if segCase == 1 && ( max(res_ECRe,res_ECRem) * 1.05 < res_ECR )
    ...
    Yp0d      = cat_vol_localstat(Yp0,Yp0>1,1,3); % cat_vol_morph(Yp0,'gdilate');
    ...
    if test2 && segCase >= 6 
      Yp0d2      = cat_vol_localstat(Yp0d,Yp0d>1,1,3); %cat_vol_morph(Yp0d,'gdilate');
    ...
    if test2
      Yp0e2      = max(2,cat_vol_localstat(Yp0e,Yp0e>1,1,2)); %cat_vol_morph(Yp0e,'gerode');
