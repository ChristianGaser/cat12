        ...
        if any(vx_red<=0.5) % 0.65
         ...
         elseif strcmp(interp,'cat_stat_nanmean') || strcmp(interp,'meannan')
          ...
          elseif strcmp(interp,'mean')
            ...
            [Rx,Ry,Rz] = meshgrid(single( ss(2):ss(2):size(T{i},2)),...
                                  single( ss(1):ss(1):size(T{i},1)),...
                                  single( ss(3):ss(3):size(T{i},3)));
            varargout{i} = cat_vol_interp3f(single(T{i}),Rx,Ry,Rz,interp);
          end
...
%           else
 ...
 %         end
    ...
    case 'dereducev'
      ...
      %switch interp
      %  case {'cat_stat_nanmean','meannan','mean','median','meanm','stdm','min','max'}
      ...
      %  otherwise
      %    [Rx,Ry,Rz]   = meshgrid(single(1:1/vx_red(2):sD(2)+1/vx_red(2)),...
      %                            single(1:1/vx_red(1):sD(1)+1/vx_red(1)),...
      %                            single(1:1/vx_red(3):sD(3)+1/vx_red(3)));
      %end
      for i=1:numel(T)  
        T{i}(isnan(T{i})) = 0; 
