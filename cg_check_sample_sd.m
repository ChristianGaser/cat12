...
if nargin < 1
  ...
  sd_name = spm_input('Output SD filename','+1','s','SD.nii');
  mean_name = spm_input('Output Mean filename','+1','s','Mean.nii');
...
%-Start progress plot
%-----------------------------------------------------------------------
mean_vol = zeros(V(1).dim(1:3));
std_vol  = zeros(V(1).dim(1:3));
...
for j=1:V(1).dim(3),
  ...
  mean_vol(:,:,j) = mean_slice;
  if ~isempty(mask)
    for i = 1:n
      dist2  = (vol(mask,i) - mean_slice(mask)).^2; 
      dist2_slice(mask) = dist2;
      std_vol(:,:,j) = std_vol(:,:,j) + dist2_slice;
      squared_distance(i) = squared_distance(i) + sum(dist2(~isnan(dist2)));
    end
...
%-----------------------------------------------------------------------
if ~isempty(sd_name)
  Vo1 = spm_write_vol(Vo1,std_vol);
  disp(['Writing ' Vo1.fname]);
end

if ~isempty(mean_name)
  Vo2 = spm_write_vol(Vo2,mean_vol);
