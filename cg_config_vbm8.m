...
native.help  = {[...
'The native space option allows you to produce a tissue class image (p*) that ',...
'is in alignment with the original/* (see Figure \ref{seg1})*/. ',...
'It can also be used for "importing" into a form that can be used with DARTEL (rp*)']};
...
'both with (mwp*) and without (wp*) modulation (see below). ',...
...
mrf    = mnu('Amap MRF weighting','mrf',{...
'no HMRF','small HMRF weighting (0.25)','medium HMRF weighting (0.5)',...
'large HMRF weighting (0.75)','maximum HMRF weighting (1.0)'},...
{0,0.25,0.5,0.75,1});
mrf.val  = {1.0};
mrf.help = {[...
...
'estimate the Maximum a posteriori (MAP). Prior probability will be iteratively ',...
'weighted to cover a wide range of different noise levels. The fix HMRF weighting ',...
'was retained for compatibiliy purposes. The iterative weighting is the recommended ',...
'option and the resulting images are indicated by the term ''_aMRF'', whereas the fix ',...
'weighting is indicated by ''_HMRF''. Parts of this version are based on an ',...
'implementation of a HMRF by Meritxell Bach Cuadra et al. (IEEE TMI 2005).']};
...
warping = branch('Warping','warp',{warpreg,affreg,affmethod,samp,warps,cleanup,bb,vox});
...
%------------------------------------------------------------------------
warping2 = branch('Warping','warp',{warps,cleanup,bb,vox});
