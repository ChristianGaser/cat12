...
global cat_stat_spm_result_ui_varargout oldGraphicsRes;

% get old figure rending settings
try
  fg = spm_figure('FindWin','Graphics');
  
  restext   = char(fg.ExportsetupWindow.restext);  
  resstr    = min(strfind( restext ,'selectedItemReminder')); 
  resstrend = resstr + [ min( strfind( restext(resstr:end),'=') )  , ...
                         min( [ strfind(restext(resstr:end),',') , ...
                                strfind(restext(resstr:end),']') ] - 1) ];
  if numel(resstrend)==2 
    if exist('oldGraphicsRes','var') && isempty(oldGraphicsRes)
      oldGraphicsRes  = restext; %restext(resstrend(1):resstrend(2)-1); 
    end
    restext         = [restext(1:resstrend(1)-1) '75' restext(resstrend(2):end)]; 
    fg.ExportsetupWindow.restext = restext;
  end
end
...
% CAT.end  
    ...
    %======================================================================
        ...
        fg = spm_figure('FindWin','Graphics');
        if ~isempty(oldGraphicsRes)
          fg.ExportsetupWindow.restext = oldGraphicsRes;
        end      
