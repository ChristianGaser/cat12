...
cat.opts.biasfwhm  = 60;                      % Bias FWHM - lower values for stronger bias fields, but look for overfitting in subcortical GM (values <50 mm)
cat.opts.samp      = 3;                       % Sampling distance - smaller 'better', but slower - maybe usefull for >6 Tesla 
...
cat.bias.nits_bias      = 8;
cat.bias.biasfwhm       = 60;
cat.bias.biasreg        = 1e-6;
cat.bias.lmreg          = 1e-6;
% realign options 
cat.realign.halfway     = 1;      % use halfway registration: 0 - no; 1 - yes
cat.realign.weight      = 1;      % weight registration with inverse std: 0 - no; 1 - yes
cat.realign.ignore_mat  = 0;      % ignore exisiting positional information: 0 - no; 1 - yes
% apply deformations options
cat.defs.interp         = 5;      % 5th degree B-spline
...
cat.extopts.brainmask    = {fullfile(spm('dir'),'toolbox','cat12','templates_1.50mm','brainmask.nii')};                 % brainmask for affine registration
cat.extopts.T1           = {fullfile(spm('dir'),'toolbox','cat12','templates_1.50mm','Template_T1_IXI555_MNI152.nii')}; % T1 for affine registration

% surface options
cat.extopts.surface      = 0;     % surface and thickness creation
...
cat.extopts.ROI          = 2;     % write csv-files with ROI data: 1 - subject space; 2 - normalized space; 3 - both (default 2)
cat.extopts.print        = 1;     % Display and print results
cat.extopts.verb         = 2;     % Verbose: 1 - default; 2 - details
cat.extopts.debug        = 1;     % debuging option: 0 - default; 1 - write debuging files 
...
cat.extopts.LAB.ON = 11; % Optical Nerv
