function cat_io_send_udp_to_server
% This function will send mac-address and version information to the SBM server
%_______________________________________________________________________
% Christian Gaser
% $Id: cat_io_send_udp_to_server.m 1355 2018-08-13 13:53:48Z gaser $
...
% replace "-" by "." for windows systems
mac = strrep(mac,'-',':');
...
if isempty(strfind(RS,'Usage'));
  info = sprintf('%s,%s,CAT%s,SPM%s,MATLAB%s,%s,%s,%s',mac,dt,rev_cat,rev_spm,version('-release'),computer,platform,RS);
else
  info = sprintf('%s,%s,CAT%s,SPM%s,MATLAB%s,%s,%s',mac,dt,rev_cat,rev_spm,version('-release'),computer,platform);
end
