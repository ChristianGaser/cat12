function cat_tst_Rusak2021(Pmethod,Presdir,datas,localstat,subset)
...
%#ok<*AGROW>
  ...
  for pi = size(Pmethod,1):-1:1
    if any(contains(Pmethod{pi,1},'T1Prep'))
      if exist(fullfile(Pmethod{pi,2}, 'surf'),'dir')
        Pmethod(pi,2) = fullfile(Pmethod(pi,2), 'surf');
      else
        Pmethod(pi,2) = fullfile(Pmethod(pi,2), sprintf('16_Rusak2021%s',subsetn), 'surf'); 
      end
    else
...
% Presdir, age, 
      ...
      for subset = 1 %1:3 
        ...
        for pi=1:size(Pmethod,1)
          ...
          for sm = {'pbt'} %,'thickness'} ... thickness not working - similar results ...
            ...
            if ~exist(fullfile(Presdir,'batches'),'dir'), mkdir(Presdir,'batches'); end
            save(fullfile(Presdir,'batches',sprintf('Rusak.%s.mat',sm{1})),'matlabbatch'); 
    ...
    for fi = 1:2
      ...
      clf, hold on; 
      fh.Position(3:4) = [600*fi 700]; %+200*(fi-1)]; 
      ...
      for pi = 1:size(Pmethod,1)
        ...
        set(gca,'FontSize',max(7,min(16,16 / (size(Pmethod,1)/12))),'XTick',0:.2:1,'XTickLabelRotation',0); 
        if fi==2
          legend off; 
          title(sprintf('%s',Pmethod{pi,1}));
          st = subtitle( strcat(strcat( ...
            ... 'n=', num2str(nnz(Pexist(:,pi)==2),'%0.0f'), ', ', ...
            'p1=', num2str(afitp1(pi)','%0.3f'), ...
            ', r=', num2str(acor(pi)','%0.3f'), ...
            ', r^2=', num2str(arsq(pi),'%0.3f') ))); 
      ...
      if fi==1
        ...
        lgd = legend( strcat( strcat(Pmethod(:,1), ' (', ...
          ...
          'p1=', num2str(afitp1','%0.3f'), ...
          ', r=', num2str(acor','%0.3f'), ...
          ', r^2=', num2str(arsq','%0.3f') , ')' ))); 
...
function matlabbatch = longreport(x)

  mi = 1; 
  matlabbatch{mi}.spm.tools.cat.tools.check_cov.data_vol = {};
  matlabbatch{mi}.spm.tools.cat.tools.check_cov.data_xml = {''};
  matlabbatch{mi}.spm.tools.cat.tools.check_cov.gap      = 3;
  matlabbatch{mi}.spm.tools.cat.tools.check_cov.c        = {};
  matlabbatch{mi}.spm.tools.cat.tools.check_cov.outdir   = {''};
  matlabbatch{mi}.spm.tools.cat.tools.check_cov.fname    = 'CATcheckdesign_';
  matlabbatch{mi}.spm.tools.cat.tools.check_cov.save     = 0;

  for si = 1:numel()
    matlabbatch{si}.spm.tools.cat.tools.long_report.data_vol        = {''};
    matlabbatch{si}.spm.tools.cat.tools.long_report.avg_vol         = {''};
    matlabbatch{si}.spm.tools.cat.tools.long_report.data_surf       = {''}; %%
    matlabbatch{si}.spm.tools.cat.tools.long_report.data_xml        = {''}; %%
    matlabbatch{si}.spm.tools.cat.tools.long_report.timepoints      = [];   %%
    matlabbatch{si}.spm.tools.cat.tools.long_report.opts.smoothvol  = 3;
    matlabbatch{si}.spm.tools.cat.tools.long_report.opts.smoothsurf = 12;
    matlabbatch{si}.spm.tools.cat.tools.long_report.opts.midpoint = 0;
    matlabbatch{si}.spm.tools.cat.tools.long_report.opts.plotGMWM = 1;
    matlabbatch{si}.spm.tools.cat.tools.long_report.output.vols   = 0;
    matlabbatch{si}.spm.tools.cat.tools.long_report.output.surfs  = 0;
    matlabbatch{si}.spm.tools.cat.tools.long_report.output.xml    = 1;
    matlabbatch{si}.spm.tools.cat.tools.long_report.printlong     = 2;
  end
end