...
%#ok<*ASGLU,*WNOFF,*WNON,*TRYNC>
  ...
  recalc  = 0; 
  mode    = 0; % modulation of each label map? .. do not work yet ... see cg_cat_defs
    ...
    for fi=1:numel(P)
      ...
      if refine
        ...
        if recalc || ( ~exist(Pwa{fi},'file') || ~exist(Pwp0{fi},'file') ) 
          fprintf('Recalculation of case %d/%d:\n',fi,numel(P));
...
% ----------------------------------------------------------------------
  ...
  matlabbatch{1}.spm.tools.cat.estwrite.opts.tpm                = ...
    {'/Users/dahnke/Neuroimaging/cat12/tpm/TPM.nii'};
...
function dict=ROIdict()
  ...
  dict.regions = { ... % specific - one case
    ...
    'Rol'            {'Rolandic'} {}
    'Pal'            {'Pallidum'} {}
    'ParHip'         {'Parahippocampus'} {}
    'ParHip'         {'Parahippocampal'} {}
    'Pla'            {'Planum Polare'} {}
    'Put'            {'Putamen'} {}
    'Rec'            {'Rectal'} {}
...
% create an spm12 compatible xml version of the csv data
  ...
  fid = fopen(fullfile(pp,['label_' ff '.xml']),'w');
