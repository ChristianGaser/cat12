...
usecom     = cfg_menu;
usecom.tag = 'usecom';
usecom.name = 'Set origin';
usecom.labels ={'use origin from header','use center of mass to set origin'};
usecom.values = {0,1};
usecom.def  = @(val)cg_vbm8_get_defaults('opts.usecom', val{:});
usecom.help = {[...
'It is often helpful to set the origin (anterior comissure) using the Display tool ',...
' before spatial registration. However, the origin can be roughly approximated by calculating the center-of-mass (COM) ',...
'of the image and to shift the image to the COM of the template. This roughly corrects differences in the ',...
'position between the image and the template and utilizes spatial normalisation. In case that spatial registration ',...
'(and segmentation) fails with this options the safest way is to define the anterior comissure before segmentation ',...
'and to set this option to ''''use origin from header''''.']};
...
opts.val = {tpm,ngaus,biasreg,biasfwhm,usecom,affreg,warpreg,samp};
opts.help = {[...
