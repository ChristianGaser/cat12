function varargout = vbm_io_csv(filename,varargin)
...
% will stand for read.
%
%   vbm_io_csv(filename,C[,sheet,pos,opt])
%   C = vbm_io_csv(filename[,sheet,pos,opt])
%
...
%
% Examples:
%   vbm_io_csv('test',{'Hallo','Welt';1,2.4})
%   C=vbm_io_csv('test.csv','','A1:C1')
%   vbm_io_csv('test',rand(3,3),'','',struct('delimiter',',','komma','.'))
%
% TODO:
...
% ______________________________________________________________________
  ...
  switch action
    ...
    otherwise
      error('MATLAB:VBM_IO_CSV:unkown_action','Unknown action ''%s''',action);
  end
end
...
% __________________________________________________________________________________________________
  ...
  for i=1:size(C1,1),
    ...
    % fprintf('%4.0f-%4.0f\n',numel(strfind(C1{i},';')),numel(C2{i}));
      ...
      fprintf('WARNING:vbm_io_csv:readcsv: Can''t read line %d!\n',i); C2{i}=cell(1,numel(C2{1})); %#ok<AGROW>
    end
  end
