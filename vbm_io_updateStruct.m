function S=vbm_io_updateStruct(S,SN,RepByEmpty,ind)
...
%
%   S = vbm_io_updateStruct(SI,SN[,RepByEmpty,ind])
%   
% WARNING:
...
% $Id$
  ...
  if numel(SN)>1
    ...
    for sni=ind
      S = vbm_io_updateStruct(S,SN(sni),RepByEmpty,sni);
    end
  else  
        ...
        if RepByEmpty || ~isempty(SN.(fnS{fnSi}))
          if isstruct(SN.(fnS{fnSi})) 
            ...
            if numel(S)<ind
              ...
              S(ind).(fnS{fnSi}) = vbm_io_updateStruct(S(1).(fnS{fnSi}),SN.(fnS{fnSi}),0);
            else
              S(ind).(fnS{fnSi}) = vbm_io_updateStruct(S(ind).(fnS{fnSi}),SN.(fnS{fnSi}),RepByEmpty);
            end
          else
