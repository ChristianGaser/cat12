function varargout = vbm_surf_render(action,varargin)
% Display a surface mesh & various utilities
% FORMAT H = vbm_surf_render('Disp',M,'PropertyName',propertyvalue)
...
switch lower(action)
    ...
    otherwise
        try
            H = vbm_surf_render('Disp',action,varargin{:});
        catch
...
if numel(findobj('Tag','VBMSurfRender','Type','Patch')) > 1
...
P = findobj('Tag','VBMSurfRender','Type','Patch');
if numel(P) == 1
    %camlight(H.light);
else
    for i=1:numel(P)
...
vbm_surf_render('Colourbar',H,toggle(get(obj,'Checked')));

%==========================================================================
function myColourmap(obj,evt,H)
vbm_surf_render('Colourmap',H,feval(get(obj,'Label'),256));
