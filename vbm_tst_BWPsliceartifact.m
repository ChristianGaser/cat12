...
%BWP image
Pp0  = '/Volumes/MyBook/MRData/vbm_tst/+RAW/BWP_Collins/SEG/p0BWPC_HC.nii';
P    = {
...
for pi=1:numel(P)
  ...
  for iae = 1:numel(art.intraSlAmp)
    ...
    for ief = 1:numel(art.interSlFreq)
      ...
      % add rand distortion
      [Yr,resTr] = vbm_vol_resize(Y,'reduceV',1,45,1);
      ...
      resTr.vx_red(3) = 1;
      Yas = vbm_vol_resize(Yas,'dereduceV',resTr);
...
bwp.dir.Yo  = '/Volumes/vbmDB/MRData/vbm12tst/results/deffiles/cg_vbm_defaults/BWPC_sliceart_uncorr/';
bwp.dir.Yc  = '/Volumes/vbmDB/MRData/vbm12tst/results/deffiles/cg_vbm_defaults/BWPC_sliceart_uncorr/';
bwp.dir.Ygt = '/Volumes/MyBook/MRData/vbm_tst/+RAW/BWP_Collins/SEG/';
bwp.dir.Yp0o = '/Volumes/vbmDB/MRData/vbm12tst/results/deffiles/cg_vbm_defaults/BWPC_sliceart_uncorr/';
bwp.dir.Yp0c = '/Volumes/vbmDB/MRData/vbm12tst/results/deffiles/cg_vbm_defaults/BWPC_sliceart_uncorr/';

Po   = vbm_findfiles(bwp.dir.Yo ,'*.nii');
Pc   = vbm_findfiles(bwp.dir.Yc ,'scorr*.nii');
Pp0c = vbm_findfiles(bwp.dir.Yp0c,'p0scorr*.nii'); 
Pp0o = vbm_findfiles(bwp.dir.Yp0o,'p0*.nii'); Pp0o = setdiff(Pp0o,Pp0c);
Pgt  = vbm_findfiles(bwp.dir.Ygt,'p0*.nii'); 

% BWP Kappa estimation
[tmp,bwp.K{1}] = vbm_tst_calc_kappa(Pp0o,Pgt);
[tmp,bwp.K{2}] = vbm_tst_calc_kappa(Pp0c,Pgt);
bwp.Kappa{1}   = mean([bwp.K{1}{2:end-2,3};bwp.K{1}{2:end-2,4}]);
bwp.Kappa{2}   = mean([bwp.K{2}{2:end-2,3};bwp.K{2}{2:end-2,4}]);

% BWP Kappa estimation
bwp.CJV{1} = vbm_tst_CJV(Po,Pgt);
bwp.CJV{2} = vbm_tst_CJV(Pc,Pgt);
...
vbm_plot_boxplot(bwp.Kgr{1},struct('ylim',[0.3 1],'names',bwp.Knames(1),'title','BWP with slice artifacts',...
  'groupnum',1 ));
%%
print(fh1,fullfile(printdir,sprintf('fig_tst_sliceart_%s',datestr(clock,'yyyymmdd'))),fig.res,fig.type);
%vbm_plot_boxplot(bwp.Kgr{2},struct('ylim',[0.85 0.95],'names',bwp.Knames(2),'title','BWP with slice artifacts',...
%  'groupnum',0));

%%
%vbm_plot_boxplot(ibsr.CJV,struct('ylim',[0 1])); % not usefull
%vbm_plot_boxplot(bwp.Kappa,struct('ylim',[0.5 1]));
...
%% IBSR results
ibsr.dir.Yo   = '/Volumes/vbmDB/MRData/vbm12tst/RAW/IBSRv1';
ibsr.dir.Yc   = '/Volumes/vbmDB/MRData/vbm12tst/results/deffiles/cg_vbm_defaults/IBSRv1_uncorr/';
ibsr.dir.Ygt  = '/Volumes/MyBook/MRData/vbm_tst/+RAW/IBSRv1/SEG/';
ibsr.dir.Yp0o = '/Volumes/vbmDB/MRData/vbm12tst/results/deffiles/cg_vbm_defaults/IBSRv1_uncorr/';
ibsr.dir.Yp0c = '/Volumes/vbmDB/MRData/vbm12tst/results/deffiles/cg_vbm_defaults/IBSRv1_uncorr/';

Po   = vbm_findfiles(ibsr.dir.Yo ,'*.nii');
Pc   = vbm_findfiles(ibsr.dir.Yc ,'scorr*.nii');
Pp0c = vbm_findfiles(ibsr.dir.Yp0c,'p0scorr*.nii'); 
Pp0o = vbm_findfiles(ibsr.dir.Yp0o,'p0*.nii'); Pp0o = setdiff(Pp0o,Pp0c);
for pi = 1:numel(Pp0o)
...
% BWP Kappa estimation
[tmp,ibsr.K{1}] = vbm_tst_calc_kappa(Pp0o,Pgto);
[tmp,ibsr.K{2}] = vbm_tst_calc_kappa(Pp0c,Pgtc);
...
% BWP Kappa estimation
ibsr.CJV{1} = vbm_tst_CJV(Po,Pgt);
ibsr.CJV{2} = vbm_tst_CJV(Pc,Pgt);
...
vbm_plot_boxplot(ibsr.Kappa,struct('ylim',[0.7 0.9+eps],'title','BWP with slice artifacts',...
