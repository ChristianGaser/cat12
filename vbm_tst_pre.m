...
%       end
  ...
  for di=1:numel(opt.subdirs)
    ...
    QATT{di} = [QATT{di}; 
     sprintf('mean (>%0.2f)',opt.FailedTh),num2cell([nanmean(resultqa{di}), nanmean(resultNF{di})]); 
     sprintf('std  (>%0.2f)',opt.FailedTh),num2cell([nanstd(resultqa{di}),  nanstd(resultNF{di})]); 
     sprintf('min  (>%0.2f)',opt.FailedTh),num2cell([nanmin(resultqa{di}),  nanmin(resultNF{di})]);
    ...
    QAT{di} = [QAT{di}, ...
      ...
      sprintf(sprintf('%%%ds: %%%d.%df %%%d.%df %%%d.%df %%%d.%df %%%d.%df | %s\n', ...
          opt.printspace(1),[repmat(opt.printspace(2),1,qan);repmat(opt.printspace(3),1,qan)],methodresstr), ...
          'mean',nanmean(resultqa{di}), nanmean(result{di})), ... 
      sprintf(sprintf('%%%ds: %%%d.%df %%%d.%df %%%d.%df %%%d.%df %%%d.%df | %s\n', ...
          opt.printspace(1),[repmat(opt.printspace(2),1,qan);repmat(opt.printspace(3),1,qan)],methodresstr), ...
          'std',nanstd(resultqa{di}), nanstd(result{di})), ... 
      ]; 
  ...
  QATF = [QATF, ...
      ...
      sprintf(sprintf('%%%ds: %%%d.%df %%%d.%df %%%d.%df %%%d.%df %%%d.%df | %s\n', ...
          opt.printspace(1),[repmat(opt.printspace(2),1,qan);repmat(opt.printspace(3),1,qan)],methodresstr), ...
          sprintf('mean (>%0.2f)',opt.FailedTh),nanmean(resultqa{di}), nanmean(cell2mat(resultNF'))), ...
      sprintf(sprintf('%%%ds: %%%d.%df %%%d.%df %%%d.%df %%%d.%df %%%d.%df | %s\n', ...
          opt.printspace(1),[repmat(opt.printspace(2),1,qan);repmat(opt.printspace(3),1,qan)],methodresstr), ...
          sprintf('std  (>%0.2f)',opt.FailedTh),nanstd(resultqa{di}), nanstd(cell2mat(resultNF'))), ...
      sprintf('%s\n',repmat('-',size(Phdr.method))),...
