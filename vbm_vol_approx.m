function TA=vbm_vol_approx(T,method,vx_vol,res,opt)
...
% 
% TA = vbm_vol_approx(T,method,vx_vol,res[,opt])
% 
% T       input image
...
% $Id$
  ...
  if (opt.hull || strcmp(method,'linear')) && ~strcmp(method,'spm')
    ...
    BMrr = vbm_vol_morph(Brr>0,'distclose',20)>0;
    BMr  = vbm_vol_resize(BMrr,'dereduceV',resTrr); 
  
    % inside hull approximation ...
    [MDr,MIr]  = vbdist(single(Tr>0),Tr==0,resTr.vx_volr); 
    TAr=Tr(MIr); TAr(Tr>0) = Tr(Tr>0); 
    TASr=vbm_vol_smooth3X(TAr,2); TAr(~BMr)=TASr(~BMr); clear TASr; 
    TAr = vbm_vol_laplace3R(TAr,BMr & ~Tr,opt.lfO); TAr = vbm_vol_median3(TAr); %,Tr>0,Tr>0,0.05); 
        %TAr = vbm_vol_laplace3R(TAr,Tr>0,opt.lfI); 
    TAr = vbm_vol_laplace3R(TAr,BMr & ~Tr,opt.lfO); 
  else
    TAr = Tr; 
    BMr = Tr>0; 
  end
  ...
  switch method
    ...
    case 'nn'
      ...
      TAr=TAr(MIr); TASr=vbm_vol_smooth3X(TAr,4); TAr(~BMr)=TASr(~BMr);  clear TASr; 
      TAr = vbm_vol_laplace3R(TAr,~BMr,opt.lfO); TAr = vbm_vol_median3(TAr,~BMr);
      TAr = vbm_vol_laplace3R(TAr,~Tr,opt.lfO); 
    case 'linear'
      TNr = TAr;
      Tr  = TAr .* BMr;
      ...
      TAr = vbm_vol_laplace3R(TAr,~BMr,opt.lfO); TAr = vbm_vol_median3(TAr,~BMr); 
      TAr = vbm_vol_laplace3R(TAr,~BMr,opt.lfO); 
    case 'spm'
...
%      ds('d2','',[1 1 1],PT{1},PT{2}/max(PT{2}(:)),TA,TAr,20)
  ...
  %{
  [TArr,BMrr,resTrr2] = vbm_vol_resize({TAr,BMr},'reduceV',vx_vol,8,8,'linear');
  TArr = vbm_vol_median3(TArr,~BMrr,~BMrr); 
  for i=1:round(8/bias),
    if mod(i,4)==0, TASrr=vbm_vol_smooth3X(TArr,8/mean(resTr.vx_volr)); TArr(~BMrr)=TASrr(~BMrr); end;
  end
  TAr  = vbm_vol_resize(TArr,'dereduceV',resTrr2); 
  %}
  %TAr = vbm_vol_smooth3X(TAr,8/mean(resTr.vx_volr)*0.1/bias).^1.05; 
  
  TA  = vbm_vol_resize(TAr,'dereduceV',resTr);
  TA  = TA*maxT;
end
function vbm_tst_pre_approx
  %%
  ...
  PM{1} = (vbdist(PM{1},true(size(PM{1})),[1.5 1 1]) + ...
          vbm_vol_smooth3X(rand(PTsize)*20,2) + vbm_vol_smooth3X(randn(PTsize)*20)) <PTsize(1)/2; 
  ...
  for i=1:numel(PT)
    PN{i} = PT{i} + vbm_vol_smooth3X(randn(PTsize)*0.4,2) + vbm_vol_smooth3X(randn(PTsize)*0.2); 
  end
  
  
  %for 
  %=vbm_vol_approx(T,method,vx_vol,res,bias)
