...
% standard approaches like N3 or the SPM and VBM proprocessings!
%
...
% ______________________________________________________________________
% Further comments: 
% - There is a private test script 'vbm_tst_BWPsliceartifact.m' that use
...
% start processing ...  
  ...
  for j = 1:length(V)
    ...
    [gx,gy,gz] = vbm_vol_gradient3(Y); 
          
    % estimate slice error and create a gradient map Yg (tissue) and Ygb
    % (background) without the critical direction 
    calc1 = vbm_stat_nanmean([ abs(gx(:)),abs(gy(:)),abs(gz(:))]) / ...
            vbm_stat_nanmean([ abs(gx(:));abs(gy(:));abs(gz(:))]);
    calc  = (calc1 > 1.01) & (calc1==max(calc1));
    
    % intern noise estimation and correction
    signal = vbm_stat_nanmedian(Y(Y(:)>vbm_stat_nanmedian(Y(:))));
    signal = vbm_stat_nanmedian(Y(Y>vbm_stat_nanmedian(Y(Y(:)>signal & Y(:)<2*signal))));  
    noise  = vbm_vol_localstat(Y,Y<signal*0.2,1,4); noise = median(noise(noise(:)>0));
    ...
    gth  = vbm_stat_nanmean(Yg(Yg(:)<vbm_stat_nanmedian(Yg(:))));
    Yth  = vbm_stat_nanmean(Y(Y>vbm_stat_nanmean(Y(Yg(:)>0 & Yg(:)<gth))));                       
    Ythw = vbm_stat_nanmean(Y(Y>vbm_stat_nanmean(Y(Yg(:)>0 & Yg(:)<gth & Y(:)>Yth*0.5))));  
    ...
    Ythb = vbm_stat_nanmean(Y(Ybg(:))); 
    %Ythw = vbm_stat_nanmedian(Y(Ywm(:))); 
     
    % estimate slice error to find the affected slice  
    calc1 = vbm_stat_nanmean([ abs(gx(Ywm(:))),abs(gy(Ywm(:))),abs(gz(Ywm(:)))]) / ...
            vbm_stat_nanmean([ abs(gx(Ywm(:)));abs(gy(Ywm(:)));abs(gz(Ywm(:)))]);
...
function [Ythi,Ythwi] = estimateWMth(Y,Yth,lb,ub)
  Ythi  = vbm_stat_nanmean(Y(Y>vbm_stat_nanmean(Y(Y(:)>0))));             % object threshold (lower boundary)
  Ythwi = vbm_stat_nanmean(Y(Y>vbm_stat_nanmean(Y(Y(:)>Ythi))));        % WM threshold for displaying
...
function [calc2,calc4] = estimateError(Yf,Ywm,Ythw)
  [gx,gy,gz] = vbm_vol_gradient3(Yf); 
  Yg    = (abs(gx) + abs(gy) + abs(gz)) ./ max(eps,Yf); 
  gth   = vbm_stat_nanmedian(Yg(Yg(:)<vbm_stat_nanmedian(Yg(:))));
  Ywm2  = vbm_vol_morph(smooth3(Yf>Ythw*0.9 & Yf<Ythw*1.2 & Yg<gth & Yg>0)>0.5,'close',2);
  calc2 = vbm_stat_nanmean([ abs(gx(Ywm(:))),abs(gy(Ywm(:))),abs(gz(Ywm(:)))]) / max(eps,...
          vbm_stat_nanmean([ abs(gx(Ywm(:)));abs(gy(Ywm(:)));abs(gz(Ywm(:)))]));
  calc4 = vbm_stat_nanmean([ abs(gx(Ywm2(:))),abs(gy(Ywm2(:))),abs(gz(Ywm2(:)))]) / max(eps,...
          vbm_stat_nanmean([ abs(gx(Ywm2(:)));abs(gy(Ywm2(:)));abs(gz(Ywm2(:)))]));
end
 ...
 if method==1
    switch dim
      case 1
        ...
        [cimg,IR1] = vbm_vol_resize({cimg},'reduceV',1,s,2,'mean');
        cimg = smooth3(cimg); 
