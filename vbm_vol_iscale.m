function [TI,varargout] = vbm_vol_iscale(T,action,vx_vol,varargin)
% VBM Prprocessing Intensity Scaling Functions
% ______________________________________________________________________
% Set of functions for intensity scaling of an image T. 
%
% actions:
%   - findhead:   [TI,H]  = vbm_vol_iscale(T,'findhead' ,vx_vol);
%   - findbrain:  [TI,B]  = vbm_vol_iscale(T,'findbrain',vx_vol);
%   - gCGW:       [TI,tp] = vbm_vol_iscale(T,'gCGW',vx_vol,...);
% 
%   [TI,varargout] = vbm_vol_iscale(T,action,vx_vol,varargin)
% 
...
% ______________________________________________________________________
  ...
  if ~exist('T','var')       
    error('MATLAB:vbm_vol_iscale:input','ERROR: Need input image!\n'); 
  else 
    T = single(T);
  end
  if ~exist('action','var')
    error('MATLAB:vbm_vol_iscale:input','ERROR: Unkown action ''%s''!\n',action);
  end
  if ~exist('vx_vol','var')
    vx_vol=ones(1,3);
  end
  ...
  switch action
    ...
    % __________________________________________________________________
      ...
      Gth = vbm_stat_nanstat1d(Gr(Mr(:)),'median');
      Mr  = vbm_vol_morph(Tr>mean(Tr(Gr(:)<Gth)) & Gr<Gth & ~(Tr<mean(Tr(:)) & Gr<Gth),'o')>0; 
      Tth = vbm_stat_nanstat1d(Tr(Mr(:)),'median');
      clear GMr TMr; 

      % background estimation
      Hr = vbm_vol_smooth3X(vbm_vol_morph(Tr>Tth*0.3,'ldc',8),1)>0.5;  %0.1 %2 % 2
      
      TI = T./Tth*0.7;
      varargout{1} = vbm_vol_resize(Hr,'dereduceV',resT);
   ...
   tic 
      ...
      [T,resTO] = vbm_vol_resize(T,'reduceV',vx_vol,1.2); vx_vol=resTO.vx_volr;
      
      % initial noise correction
      TSDs = [round(size(TO) * 2/5);round(size(TO) * 3/5)]; 
      TSD  = single(TO(TSDs(1):TSDs(2),TSDs(3):TSDs(4),TSDs(5):TSDs(6)));
      TSD  = TSD/vbm_stat_nanstat1d(TSD,'median');
      [gx,gy,gz] = vbm_vol_gradient3(TSD); GTSD=abs(gx)+abs(gy)+abs(gz); 
      TSD  = vbm_vol_localstat(TSD,GTSD<0.2,1,4);
      n1   = vbm_stat_nanstat1d(TSD(TSD>0),'mean');
      nc   = min(1.0,max(1/2,n1*10));
      TS   = vbm_vol_smooth3X(single(T),nc);
      clear TSD TSDs nc GTSD gx gy gz; 
      % gradient map
      [gx,gy,gz] = vbm_vol_gradient3(TS); G=abs(gx)+abs(gy)+abs(gz); 
      ...
      [Tr,Gr] = vbm_vol_resize({max(0,TS-(G.*TS)),G},'reduceV',vx_vol,4);
      minT = max(0,min(Tr(:))); T=T-minT; TS=TS-minT; TO=TO-minT; Tr=Tr-minT; 
      Mr   = Gr<mean(Gr(:)) & Tr>mean(Tr(:));                                                  
      Gth  = vbm_stat_nanstat1d(Gr(Mr(:)),'median');
      Mr   = vbm_vol_morph(Tr>mean(Tr(Gr(:)<Gth)) & Gr<Gth/2 & ~(Tr<mean(Tr(:)) & Gr<Gth/2),'o')>0;
%      Tth  = vbm_stat_nanstat1d(Tr(Mr(:)),'median');
      ...
      Tth = peak(TS(M2>0)/M2max)*M2max*0.5 + 0.5*vbm_stat_nanstat1d(TS(M2(:)),'median');
      M2  = vbm_vol_morph(smooth3(TS>Tth*2/3 & TS<Tth*1.6 & G<Gth/3)>0.5,'l');
      TSD = vbm_vol_localstat(T./Tth,M2,1,4); 
      n2  = vbm_stat_nanstat1d(TSD(TSD>0),'mean');
      ...
      % background estimation
      [Hr,resTr] = vbm_vol_resize(TS./G>0.5 & TS/Tth>0.2,'reduceV',vx_vol,8,8,'nearest');
      Hr = vbm_vol_morph(Hr,'dc',3);
      H = vbm_vol_resize(Hr,'dereduceV',resTr)>0.5; clear Hrr
      ...
      D  = vbm_vol_resize(vbm_vol_smooth3X(Dr),'dereduceV',resT);
  %  toc  
    ...
    tic
      ...
      if it>=1
        ...
        WIrr = vbm_vol_approx(WIrr,'nh',resT1.vx_volr,8); WIrr = vbm_vol_smooth3X(WIrr,max(2,min(4,Tth/std(WIrr(TSXrr(:)))))); 
        WIr  = vbm_vol_resize(WIrr,'dereduceV',resT2); clear WIrr; 
        ...
        [GWMrr,resT2] = vbm_vol_resize(GWMr,'reduceV',resT1.vx_volr,8,32,'meanm'); TSXrr=GWMrr>0;
        for i=1:4, GWMrr = vbm_vol_localstat(GWMrr,GWMrr>0,2,1); end
        GWMrr = vbm_vol_approx(GWMrr,'nh',resT2.vx_volr,8); GWMrr = vbm_vol_smooth3X(GWMrr,max(2,min(4,Tth/std(GWMrr(TSXrr(:))))));
        GWMr = vbm_vol_resize(GWMrr,'dereduceV',resT2); clear GWMrr;
        ...
        [GWMrr,resT2] = vbm_vol_resize(GWMr,'reduceV',resT1.vx_volr,8,32,'meanm'); TSXrr=GWMrr>0;
        for i=1:2, GWMrr = vbm_vol_localstat(GWMrr,GWMrr>0,2,1); end
        GWMrr = vbm_vol_approx(GWMrr,'nh',resT2.vx_volr,8); GWMrr = vbm_vol_smooth3X(GWMrr,max(2,min(4,Tth/std(GWMrr(:)))));
        GWMr = vbm_vol_resize(GWMrr,'dereduceV',resT2); clear GWMrr;
  
        
        % final inital maximum based bias correction WI
        TSXOr = vbm_vol_morph(Gr<Gth*2 & TSr./WIr<1.2 & TSr>min(WIr*0.9,(GWMr*0.5+0.5*WIr)) & TSr./WIr<GWMr*2,'l') .* TSr; TSXOr=vbm_vol_median3(TSXOr,TSXOr>0);
        [WIrr,resT2] = vbm_vol_resize(TSXOr,'reduceV',resT1.vx_volr,4,32,'max'); TSXrr=WIrr>0;
        WIrr = vbm_vol_localstat(WIrr,WIrr>0,1,3);
        WIrr = vbm_vol_approx(WIrr,'nh',resT2.vx_volr,8); WIrr = vbm_vol_smooth3X(WIrr,max(2,min(4,Tth/std(WIrr(TSXrr(:))))));
        WIr  = vbm_vol_resize(WIrr,'dereduceV',resT2); clear WIrr;
        ...
        Brr  = L==maxL; 
        Brr  = vbm_vol_morph(Brr | (vbm_vol_morph(Brr>0.5,'d',round(4 / mean(resT4.vx_volr))) & Trr>1/2 & Trr<1.2),'dc',10); 
        Br   = vbm_vol_resize(vbm_vol_smooth3X(Brr),'dereduceV',resT4)>0.5; clear Trr Brr;
      ...
      % ----------------------------------------------------------------
        ...
        GM  = B & T3{2};  
        [T3r,resT3] = vbm_vol_resize(T3{2},'reduceV',resT1.vx_volr,4,32,'max');
        GMC = B & vbm_vol_resize(vbm_vol_smooth3X(vbm_vol_morph(T3r,'c',4),0.5),'dereduceV',resT3)>0.5;  
        WM  = vbm_vol_morph(B & TI>0.9  & TI<1.5 & G<Gth*1.5,'l'); %,'d');& TS./WI>GWM
        WM  = WM | (~GM& GMC & TI<1.5 & G<Gth*1.5 & vbm_vol_smooth3X(WM,2)>0.1) & ~T3{2}; % & TS./WI>GWM 
  ...
  %  toc,tic 
    ...
    %%  
      %T   = vbm_vol_resize(T  ,'dereduceV',resTO);
      G   = vbm_vol_resize(G  ,'dereduceV',resTO);
      B   = vbm_vol_resize(B  ,'dereduceV',resTO)>0.5;
      WI3 = vbm_vol_resize(WI3,'dereduceV',resTO);
      H   = vbm_vol_resize(H  ,'dereduceV',resTO)>0.5;
      ...
      TSD = vbm_vol_localstat(TI,WM,1,4); 
      n3  = vbm_stat_nanstat1d(TSD(TSD>0),'mean');
      nc  = min(0.9,max(0.3,n3*10));
      BV   = B & ((TI>7/6 & ~vbm_vol_morph(TI>5/6,'l')) | TI>9/6); BV = 2*vbm_vol_morph(BV,'d') - BV;
    ...
    case 'test_findhead_findbrain' 
      ...
      if isempty(T), T = vbm_io_checkfilelist(spm_select(Inf,'image','select raw images')); end
      V = spm_vol(T); 
...
%
      if ndims(varargin{1})>2
        if max(varargin{1})==3
          for i=1:3, tp2=vbm_stat_nanstat1d(T(varargin{1}(:)==i),'median'); end
          ...
          [gx,gy,gz] = vbm_vol_gradient3(T); G=abs(gx)+abs(gy)+abs(gz);
          G=G./T; G(isinf(G) | isnan(G) | G<0)=0; clear gx gy gz;
          tpa = vbm_stat_nanstat1d(T(B & G<0.2 & T>mean(T(B(:)>0))),'median'); T = T / tpa; 

          [Tr,Gr,Br] = vbm_vol_resize({T,G,B},'reduceV',vx_vol,2);

          
          % inital tissues values
          WMr    = Br & Gr<0.2 & Tr>0.9 & Tr<1.5;                   
          tp0(3) = vbm_stat_nanstat1d(Tr(WMr(:)),'median');
          
          GMr    = Br & Gr<0.3 & Tr<0.9 & Tr>0.4;                   
          tp0(2) = vbm_stat_nanstat1d(Tr(GMr(:)),'median');
          
          CMr    = Br & Gr<0.4 & Tr<max(0.1,tp0(2)-diff(tp0(2:3))); 
          tp0(1) = vbm_stat_nanstat1d(Tr(CMr(:)),'median');
          ...
          tp1(3) = vbm_stat_nanstat1d(Tr(WMr(:)),'median');
          
          GMr    = Br & Gr<0.2 & Tr>mean(tp0(1:2)) & Tr<mean(tp0(2:3));      
          tp1(2) = vbm_stat_nanstat1d(Tr(GMr(:)),'median');
          
          CMr    = Br & Gr<0.4 & Tr<mean(tp0(1:2));                          
          tp1(1) = vbm_stat_nanstat1d(Tr(CMr(:)),'median');
...
%{ 
% working version
function [TI,T3th,T3r] = vbm_vol_CGWscale(T,G,B,Gth,vx_vol)
  %% T=vbm_vol_smooth3X(T,0.5);

  % Los gehts mit dem WM, das wir ja nun schon durch die Biaskorrektur
  % kennen und das sich um 1.00 bewegt.
  T7{6}   = vbm_vol_morph(B & G<Gth/2 & T>0.9 & T<=1.3,'l');        
  T7th(6) = min(1.3,max(0.95,vbm_stat_nanstat1d(T(T7{6}),'median'))); 
  ...
  T7{5}   = (G>Gth & G<0.5 & B & vbm_vol_morph(T>T7th(6),'d',1) & T<T7th(6));
  T7th(5) = max(0.75*T7th(6),min(0.95*T7th(6),vbm_stat_nanstat1d(T(T7{5}),'median'))); 
  T7th(7) = 2*T7th(6) - T7th(5);

  % Nun zum GM das wir durch 
  T7{4}   = G>Gth/2 & G<0.5 & B & T>0.3 & T<T7th(5);
  T7th(4) = max(0.5*T7th(6),min(0.90*T7th(6),vbm_stat_nanstat1d(T(T7{4}),'median'))); 
  T7{4}   = G<Gth*2 & B & T>T7th(4)-std(T(T7{4}(:))) & T<(T7th(4)*0.2 + T7th(6)*0.8);
  T7th(4) = max(0.5*T7th(6),min(0.90*T7th(6),vbm_stat_nanstat1d(T(T7{4}),'median'))); 
   
  T7{3}   = (G>Gth*2 & G<1 & B & vbm_vol_morph(T<T7th(4)*0.95 & B,'d',1) & T<T7th(4)*0.95);
  T7th(3) = max(0.25*T7th(6),min(0.70*T7th(6),vbm_stat_nanstat1d(T(T7{3}),'median'))); 
  
  % CSF
  T7{2} = (G<Gth/2 | (G.*T<Gth*2 & B & T<T7th(2))) & B & T<(T7th(4)/2);
  if sum(T7{2}(:))>10
    T7{2}   = vbm_vol_morph(T7{2},'l');
    T7th(2) = max(0.1*T7th(6),min(0.60*T7th(6),vbm_stat_nanstat1d(T(T7{2}),'median'))); 
  else
  ...
  TI   = vbm_vol_iscale(T,'gCGW',vx_vol,T3th); 
