...
% 2013/04
%
% $Id: vbm_pre_partvol.m 469 2013-04-15 14:51:10Z dahnke $
  ...
  WM = single(vbm_vol_morph(mgT>2.25 & mgT<3.25 & p0T>1.5,'lo',1));
  WM(~WM & (mgT<2.5 | ~p0T>0.5 | mgT>3.5))=nan; WM=vbm_vol_downcut(WM,mgT*0.9,+0.02); WM(isnan(WM))=0;
  WM = single(smooth3(WM)>0.5); 
  WM(~WM & (mgT<2.0 | ~p0T>0.5 | mgT>3.5))=nan; WM=vbm_vol_downcut(WM,mgT*0.9,-0.02); WM(isnan(WM))=0;
  WM = WM | p4ANS==opt.LAB.BG(1) | p4ANS==opt.LAB.TH(1) | p4ANS==opt.LAB.MB(1) | p4ANS==opt.LAB.BS(1) ...
          | p4ANS==opt.LAB.CB(1) | p4ANS==opt.LAB.CB(1) | p4ANS==opt.LAB.VT(1) | p4ANS==opt.LAB.ON(1);
  ...
  l1T(p4ANS==opt.LAB.BG(1) & l1T==0 & mgT>1.75 & mgT<2.75 & mgT<2.9) = opt.LAB.BG(1);          % basal ganglia
  l1T(p4ANS==opt.LAB.TH(1) & mgT>1.75 & mgT<2.75 & mgT<2.9) = opt.LAB.TH(1);                   % hypothalamus
  l1T(p4ANS==opt.LAB.HC(1) & mgT>1.75 & mgT<2.75 & mgT<2.9) = opt.LAB.HC(1);                   % hippocampus
  for s=1:2
    l1T(vbm_vol_morph(l1A==opt.LAB.VT(s) & mgT<1.75,'lab') & mgT<1.25) = opt.LAB.VT(1);     % ventricle
  end

  % refinement of WM 
  l1T(p4ANS==opt.LAB.CT(1) & l1T==0 & vbm_vol_morph(mgT>2 & l1T==1,'labopen',1)) = opt.LAB.CT(1);  
  l1T(p4ANS==opt.LAB.BG(1) & ~vbm_vol_morph(p4ANS==opt.LAB.BG(1),'erode',1) & ...
  ...
  l1T(l1T==opt.LAB.BV(1) | isinf(l1T))=0; l1T((mgT<=1 | ~p0T) & l1T==0) = -inf; 
  [l1T,D]=vbm_vol_downcut(l1T,mgT,0.1,resTr.vx_volr); 
  l1T((p4ANS==opt.LAB.BV(1) | D>500) & mgT>1.5 & ~vbm_vol_morph(mgT>2.5,'lab') & ...
    (p4ANS~=opt.LAB.CB(1) | p4ANS~=opt.LAB.HD(1)))=opt.LAB.BV(1);                         % adding blood vessels again
  l1T(l1T==opt.LAB.BG(1) & ~(p4ANS==opt.LAB.BG(1)|p4ANS==opt.LAB.VT(1)))=opt.LAB.CT(1); % basal ganglia (avoid overgrowing)
  ...
  M3 = p4ANS==opt.LAB.BG(1)|p4ANS==opt.LAB.VT(1)|p4ANS==opt.LAB.TH(1);
  ...
  l1T(smooth3(p0T)<1 & mgT>1) = opt.LAB.HD(1); 
  l1T(vbm_vol_morph(l1T>0,'lc',5) & l1T<=0 ) = opt.LAB.HD(1); 
  ...
  [MF,l1T] = vbm_vol_resize({MF,l1T},'dereduceBrain',BB);
